...
module edge_fpu_alu #(parameter GPR_WIDTH = 64, parameter PREDECODED = 0) (
  ...
  generate if(PREDECODED) begin: decoded_input
    ...
    edge_fpu_id_decode #(.GPR_WIDTH(GPR_WIDTH)) decode(.inst(issue_inst),.frm(issue_frm),.control(control));
  end endgenerate
  wire [3:0] units;
  wire [5:0] fmac_ctrl;
  wire [3:0] misc_sel;
  ...
  wire cvt_op=units[1];
  wire misc_op=units[0];
  assign issue_legal=fmac_op||slow_op||cvt_op||misc_op;
  ...
  edge_fpu_misc #(.SEQ_ID_WIDTH(1),.EPOCH_WIDTH(1),.VALUE_WIDTH(GPR_WIDTH)) misc(
    ...
    .misc_issue_op(misc_sel),.misc_issue_fmt(fmt),.misc_issue_rm(rm),
  ...
  edge_fpu_cvt #(.SEQ_ID_WIDTH(1),.EPOCH_WIDTH(1),.VALUE_WIDTH(GPR_WIDTH),.GPR_WIDTH(GPR_WIDTH)) cvt(
    ...
    .cvt_issue_op(cvt_sel),.cvt_issue_int_type(int_type),.cvt_issue_rm(rm),
...
module edge_fpu_id_decode #(parameter GPR_WIDTH=32)(
 input wire [31:0] inst,input wire [2:0] frm,output wire [31:0] control);
  ...
  wire slow_div=op_fp&&(inst[31:27]==5'b00011);
  wire slow_sqrt=op_fp&&(inst[31:27]==5'b01011)&&(rs2==0);
  wire slow_op=(slow_div||slow_sqrt)&&rounding_rm_valid;
  wire misc_sgnj=op_fp&&(inst[31:27]==5'b00100)&&(funct3<=2);
  wire misc_minmax=op_fp&&(inst[31:27]==5'b00101)&&(funct3<=1);
  wire misc_cmp=op_fp&&(inst[31:27]==5'b10100)&&(funct3<=2);
  wire misc_class=op_fp&&(inst[31:27]==5'b11100)&&(rs2==0)&&(funct3==1);
  // External moves support S/H only; no FP64 payload conversion hardware.
  wire misc_move_fmt=(inst[26:25]==2'b00)||(inst[26:25]==2'b10);
  wire misc_mvx=misc_move_fmt&&op_fp&&(inst[31:27]==5'b11100)&&(rs2==0)&&(funct3==0);
  ...
  wire cvt_f2i=op_fp&&(inst[31:27]==5'b11000)&&(rs2[4:2]==0)&&
    ((GPR_WIDTH != 32)||!rs2[1]);
  wire cvt_i2f=op_fp&&(inst[31:27]==5'b11010)&&(rs2[4:2]==0)&&
    ((GPR_WIDTH != 32)||!rs2[1]);
  wire cvt_op=(inst[26:25]!=2'b11)&&rounding_rm_valid&&
  ...
  always @* begin
    ...
    end else if(inst[31:27]==5'b00000) begin
      fmac_func[15:12]=4'b0010; fmac_func[7]=1; fmac_func[4]=1;
    end else if(inst[31:27]==5'b00001) begin
      fmac_func[15:12]=4'b0010; fmac_func[7]=1; fmac_func[3]=1;
    end else fmac_func[6:4]=3'b010;
  end


 wire add_mode=fmac_func==20'b0000_0010_0000_1001_0000;
 wire sub_mode=fmac_func==20'b0000_0010_0000_1000_1000;
 wire mul_mode=fmac_func==20'b0000_0000_0000_0010_0000;
