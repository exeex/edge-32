module edge_fpu_slow #(
  ...
  edge_fpu_div_iter div_iter (
    .clk(forever_cpuclk), .cancel(slow_cancel),
  ...
  wire ingress_special = slow_issue_sqrt
    ? (slow_issue_src0[31] || (&slow_issue_src0[30:23]) ||
       !(|slow_issue_src0[30:0]))
    : ((&slow_issue_src0[30:23]) || (&slow_issue_src1[30:23]) ||
       !(|slow_issue_src0[30:0]) || !(|slow_issue_src1[30:0]));
  // Only transaction control has reset. Payload is meaningful with valid.
  always @(posedge forever_cpuclk or negedge cpurst_b) begin
    if (!cpurst_b) begin
      ...
      slow_complete_valid <= 1'b0;
      case (state_r)
        ST_IDLE: if (slow_issue_valid) begin
          busy_r <= 1'b1;
          state_r <= ingress_special ? ST_ROUND : ST_NORM_A;
        end
        ST_NORM_A: if (sig_a_r[23])
          state_r <= sqrt_r ? ST_PREP : ST_NORM_B;
        ST_NORM_B: if (sig_b_r[23]) state_r <= ST_PREP;
        ST_PREP: state_r <= sqrt_r ? ST_SQRT : ST_DIV;
        ST_DIV: if (div_last) state_r <= exp_z_r < 0 ? ST_DENORM : ST_ROUND;
        ST_SQRT: if (sqrt_last) state_r <= ST_ROUND;
        ST_DENORM: if (round_exp_r == -1) state_r <= ST_ROUND;
        ST_ROUND: begin
          busy_r <= 1'b0;
          state_r <= ST_IDLE;
          slow_complete_valid <= 1'b1;
        end
        default: begin state_r <= ST_IDLE; busy_r <= 1'b0; end
      endcase
    end
  end
  // Each consumed payload field is initialized by issue, PREP or final capture.
  // Do not qualify numeric register data/enables with reset.
  always @(posedge forever_cpuclk) begin
    if (!slow_cancel) begin
      case (state_r)
        ST_IDLE: begin
