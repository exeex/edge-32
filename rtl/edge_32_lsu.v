...
// One RV32 memory instruction at a time: capture, request, wait for completion.
module edge_32_lsu #(
  ...
  input  wire [31:0] op_base,
  input  wire [31:0] op_offset,
  input  wire [31:0] op_store_data,
  ...
  wire [31:0] incoming_addr = op_base + op_offset;
  wire [1:0] incoming_size = op_fp && (op_funct3[2:1] == 2'b11) ?
                             2'b00 : op_funct3[1:0];
  wire incoming_unsupported = !op_fp && (op_funct3[1:0] == 2'b11);
  ...
  wire [63:0] shifted_store = {32'd0, store_data_q} << (byte_offset * 8);
  wire [7:0] base_strobe = size == 2'd0 ? 8'h01 :
                           size == 2'd1 ? 8'h03 : 8'h0f;
  ...
  always @* begin
    case (size)
      2'd0: formatted_load = funct3_q[2] ? {24'd0, shifted_load[7:0]} :
                                           {{24{shifted_load[7]}}, shifted_load[7:0]};
      2'd1: formatted_load = funct3_q[2] ? {16'd0, shifted_load[15:0]} :
                                           {{16{shifted_load[15]}}, shifted_load[15:0]};
      default: formatted_load = shifted_load[31:0];
    endcase
  end
