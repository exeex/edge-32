...
constexpr uint8_t kDmaCsrXStride = 0;
constexpr uint8_t kDmaCsrXCount  = 1;
constexpr uint8_t kDmaCsrYStride = 2;
constexpr uint8_t kDmaCsrYCount  = 3;
...
{
    // RV32 carries each DMA XY field in its own GPR-backed CSR command.
    edge32::emit<edge32::command::dma_setcsr, edge32::kDmaCsrXStride>(
        static_cast<uint32_t>(stride));
    edge32::emit<edge32::command::dma_setcsr, edge32::kDmaCsrXCount>(
        static_cast<uint32_t>(count));
}
static inline void edge_dma_sety(uintptr_t stride, uintptr_t count)
{
    edge32::emit<edge32::command::dma_setcsr, edge32::kDmaCsrYStride>(
        static_cast<uint32_t>(stride));
    edge32::emit<edge32::command::dma_setcsr, edge32::kDmaCsrYCount>(
        static_cast<uint32_t>(count));
}
